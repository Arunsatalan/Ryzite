...
import { INITIAL_PROJECTS, INITIAL_SERVICES } from '../../src/data/initialData';
...
export default function PortfolioPage() {
  const jsonLd = {
    ...
    itemListElement: INITIAL_PROJECTS.map((proj, idx) => ({
      '@type': 'ListItem',
      position: idx + 1,
      item: {
        '@type': 'CreativeWork',
              ...
              >
                <div className="lg:col-span-6 space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-semibold text-slate-400">Client: {project.client}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {project.techStack.map((tech, tIdx) => (
                      <span key={tIdx} className="text-xs font-bold text-slate-600 bg-slate-200/60 px-3 py-1 rounded-lg">
                        {tech}
                      </span>
                    ))}
                  </div>
